...
function fillBoard(board) {
    ...
    for (let i = 0; i < board.tiles.length; i++) {
        ...
        tileDom.innerHTML = null;  // Clear previous contents
        ...
        img.src = tileObj.face;

        // Create title element
        const titleDiv = document.createElement('div');
        titleDiv.className = 'tile-title';
        titleDiv.innerText = tile_title;

        // Create video element programmatically
        const video = document.createElement('video');
        video.loop = true;
        video.muted = true; // Keep muted as per autoplay policy
        const source = document.createElement('source');
        source.src = tileObj.clips[0];
        source.type = 'video/mp4';
        video.appendChild(source);

        // Append elements to the tile DOM
        tileDom.appendChild(img);
        tileDom.appendChild(titleDiv);
        ...
        const myCallback = () => {
            if (configPane.classList.contains("menu-expanded")) {
                ...
                // Change the video source and load it
